...
if(class_exists('age_restriction_Validation') != true) {
	class age_restriction_Validation {

		const VERSION = 1;
		const ALIAS = 'age_restriction';
			...
			if($sum_hash != $hash){
				...
				if( count($results) > 0 ){
					foreach ($results as $key => $value){
						if( get_option('age_restriction_do_activation_redirect', false) == false ){
