...
$(document).ready(function() {
    ...
    if( !ageRestriction_isMobile.any && $(window).height() <= 980 ) {
		$('#wrapper').css({
			minHeight: $('.ara-box').outerHeight() + 50
		});
	}
...
function setBirthdayAndSubmit(birthday, format, splitter) {
	...
	if( format == 'MM-DD-YYYY' ) {
		if( jQuery('input[name="age_restriction_day"]').length > 0 && jQuery('input[name="age_restriction_month"]').length > 0 && jQuery('input[name="age_restriction_year"]').length > 0 ) {
		...
		}else if( jQuery('select[name="age_restriction_day"]').length > 0 && jQuery('select[name="age_restriction_month"]').length > 0 && jQuery('select[name="age_restriction_year"]').length > 0 ) {
			jQuery('select[name="age_restriction_day"] option[value="'+birthday[1]+'"]').attr('selected', 'selected');
			jQuery('select[name="age_restriction_month"] option[value="'+birthday[0]+'"]').attr('selected', 'selected');
			jQuery('select[name="age_restriction_year"] option[value="'+birthday[2]+'"]').attr('selected', 'selected');
		...
		if( jQuery('input[name="age_restriction_day"]').length > 0 && jQuery('input[name="age_restriction_month"]').length > 0 && jQuery('input[name="age_restriction_year"]').length > 0 ) {
			...
			jQuery('select[name="age_restriction_day"] option[value="'+birthday[2]+'"]').attr('selected', 'selected');
			jQuery('select[name="age_restriction_month"] option[value="'+birthday[1]+'"]').attr('selected', 'selected');
			jQuery('select[name="age_restriction_year"] option[value="'+birthday[0]+'"]').attr('selected', 'selected');
			
		}
	}
	
	jQuery('select[name="age_restriction_day"], select[name="age_restriction_month"], select[name="age_restriction_year"]').each(function(){
		var that = $(this),
			parent = that.parent('div'),
			styledSelect = parent.find(".styledSelect");
		
		styledSelect.text( that.val() );
	});
