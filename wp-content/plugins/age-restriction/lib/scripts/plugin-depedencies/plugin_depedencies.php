...
if (class_exists('aaTeamPluginDepedencies') != true) {
    class aaTeamPluginDepedencies
    {
	    ...
	    * Singleton pattern
	    *
	    * @return age_restrictionSpinner Singleton instance
	    */
		...
		public function createDepedenciesPage() {
			add_menu_page(
				$this->the_plugin->pluginName . __( ' Depedencies', 'age-restriction' ),
				$this->the_plugin->pluginName . __( ' Depedencies', 'age-restriction' ),
				'manage_options',
		...
		public function depedencies_plugin_redirect_valid() {
			delete_option('age_restriction_depedencies_is_valid');
			wp_redirect( get_admin_url() . 'admin.php?page=age_restriction' );
		}
		
		public function depedencies_plugin_redirect() {

			delete_option('age_restriction_depedencies_do_activation_redirect');
			wp_redirect( get_admin_url() . 'admin.php?page=age_restriction' );
		}
		...
		public function verifyDepedencies() {
			...
			?>
			<div class="age_restriction-message">
				 All of the bellow libraries must be enabled in order for our plugin to function right!
			</div>

			<input type="button" value="Re-Check Depedencies" class="age_restriction-button blue age_restriction-depedencies-check" style="margin-top: 10px;">
			<?php
