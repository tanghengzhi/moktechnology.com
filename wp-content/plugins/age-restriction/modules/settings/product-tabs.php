...
class amzAffReviewProductTab {
	...
	public function __construct() {
		...
		$this->tab_title = __('Amazon Customer Reviews', 'age-restriction');
		
		$config = @unserialize( get_option( $age_restriction->alias . '_amazon' ) );
		
		if( $config['show_review_tab'] == 'yes') add_action( 'init', array( $this, 'init' ));
	...
	public function custom_product_tabs( $tabs = array() ) {
		global $product, $age_restriction;
	...
	function product_review_tab( $tab )
	{
		if($this->product_has_custom_tabs($product)) {
			...
			if( trim($url) != "" ){
				...
				if( trim($params['exp']) != "" ){
					...
					if( time() > $expire_on ){
						...
						global $post, $age_restriction;
						
						$post_id = (int)$post->ID > 0 ? $post->ID : 0;
						if( $post_id > 0 ){
							$new_url = $age_restriction->amzHelper->updateProductReviews( $post_id );
	...
	function product_cross_sell_tab( $tab )
	{
		if($this->product_has_custom_tabs($product)) {
			if($this->product_has_custom_tabs($product)) {
	...
	private function product_has_custom_tabs($product) {
		...
		if($this->tab_data === false) {
			$reviews = maybe_unserialize( get_post_meta($product->id, 'amzaff_woo_product_tabs', true) );
			$this->tab_data[] = $reviews[0];
			 
			if( isset($cross_selling) && count($cross_selling) > 1 ){
				$this->tab_data[] = array(
					'id' => $age_restriction->alias . '_cross_selling_tab',
